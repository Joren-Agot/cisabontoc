...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Get the current date and time for processed date
    $processedDate = date('Y-m-d H:i:s');  // Current date and time

    // Update status in work_requests table
    $stmt = $conn->prepare("UPDATE work_requests SET status = ?, processed_date = ? WHERE id = ?");
    $stmt->bind_param('ssi', $status, $processedDate, $id);  // Bind the processed date
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt4->execute()) {
                ...
                if ($status == 'Processed' || $status == 'Declined') {
